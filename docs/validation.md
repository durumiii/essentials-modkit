...
## scope 도입 후 실설치본 진단 (2026-08-04)

부분 매니페스트가 실제로 게임 원본을 외래로 몰지 않는지 실설치본에서 확인했다.
패키지 조립기(`pokemon-z/share/make_package.py`)가 새로 동봉하는
`scope="partial"` 매니페스트(222개 파일)로, 실제 설치본
`/mnt/d/Game/Pokemon Z/V2.18`을 진단만 했다 — 격리는 돌리지 않았고 설치본에
쓰기도 하지 않았다.

```
$ uv run python -m modkit.cli diagnose "/mnt/d/Game/Pokemon Z/V2.18" \
    -m ".../dist/포켓몬Z 한글패치 v5/manifest.json" --store /mnt/d/GameVault/mods
원본 일치 182 · 아는 변경 1 (한글패치 통합) · 외래 0 · 누락 39 · 추적 밖 18927
깨끗해요 — 외래 파일이 없어요.
5:02 total   (종료 코드 0)
```

외래 0과 추적 밖 18,927이 이 변경의 핵심 관측이다. scope 이전이라면 그 18,927개
(게임 원본·유저 데이터)가 전부 외래로 서고, GUI의 기본 버튼 [격리 후 계속]이
그것을 통째로 옮겨 게임을 부쉈을 자리다.

누락 39는 예상 밖이었다. 전부 `번역표/` 아래 문서·도구 파일이다 — 게임이 읽지
않는 재료라 받는 사람이 게임 폴더에 안 옮겨도 정상인데, 매니페스트가 목록에
담고 있어 진단이 "빠진 파일 39개"로 경고한다. 게임 자체의 파일은 하나도 빠지지
않았다. 고치지 않고 기록만 한다(아래 어긋난 관측 7번).
...
7. 동봉 매니페스트가 `번역표/` 39개 파일을 목록에 담는다. 게임이 읽지 않는 재료라
   안 옮겨도 정상인데 진단은 누락으로 세고, GUI는 그 상태를 주황 신호등으로 보여
   준다. 패키지 조립 때 `번역표/*`를 제외 패턴에 넣으면 사라질 어긋남이다
   (2026-08-04 실측, 고치지 않음).
...
# 실기 검증 2차 — 다른 게임 실설치본과 층 규칙 (2026-08-04 밤)

무대는 D:\Game의 실설치본(사용자 선택 — 사본 없이 직접). 왕복은 설치 전후
대상 파일의 sha256 대조.

## 게임 3종 왕복

- **Pokémon Nova V2.0** — 한글패치 v1.0(에셋 7 + 묶음 통짜). 첫 왕복에서
  **실물 버그**: 손패치로 이미 있던 동일 파일(999_Main.rb·폰트)을 설치가 백업
  없이 건너뛰고, 제거가 "내가 놓은 것"으로 오인해 삭제(즉시 복구·수리 —
  이미 있던 자리엔 내용이 같아도 .orig을 남긴다). 수리 후 왕복 해시 동일.
- **Pokémon: Wishing Star v1.0.8** — 주입형 2종(Better Following·Type Helper,
  v1.0.7 수확 카드). 무경고 설치, 왕복 해시 동일. 주입 백업(PluginScripts.rxdata.orig)은
  제거 후에도 남는다 — 뜻-왕복 재직렬화의 안전망으로 의도된 동작.
- **Pokémon Añil v4.1.0** — 이로치 확률 설정(묶음+코어 통짜, 강행 경유). 왕복
  해시 동일. 부수 발견: 게임이 플러그인 52개를 동봉해 씻김 경고가 화면을 삼킴
  → 이름 4개+"외 N개" 집계로 수리. 이로치는 한글패치 1.0.0y의 정식 후속판
  (총 770섹션 중 3개만 차이, README가 순차 덮어쓰기를 지시)임을 섹션 대조로 실측.

## Wishing Star v1.0.7 즉사 크래시 규명

증상: 실행 즉시(실제로는 첫 마우스/터치 입력에) 창이 꺼짐. 세이브·모드 제거와
무관. 합성 입력 재현기로 확정: **mkxp.json의 integerScalingActive: true + 창모드
+ 마우스 클릭 = 0xC0000094(정수 0 나누기)**. 키보드는 무해, 입력 없이는 안 죽음
— 입력 없는 관찰로는 이 크래시를 원리적으로 못 본다(세이브→LF 줄끝으로 두 번
오판한 원인). 그 설정은 8/4 새벽 실험 세션이 실물에 남긴 잔재(_probe 사본과
mkxp.json.orig 생성 시각으로 추적). 순정 복원으로 해소.

## NTFS 콜론 함정

기준선 파일명의 `::`(예: Battle::Scene__x.rb)는 NTFS에 못 들어간다 — WSL은
U+F03A로 몰래 치환해 WSL에선 멀쩡해 보이고, Windows 네이티브 exe는 깨진 이름을
읽어 검사가 전멸("~가 없어요", GUI엔 U+F03A가 << 비슷하게 표시). 코드포인트
직독(61498=0xF03A)으로 실측. 쓰기는 %3A 치환, 읽기는 둘 다 복원으로 수리,
두 보관소 파일명 이주 후 Windows 쪽 검증 0건.

## 층 규칙 왕복 (Windows 네이티브)

샌드박스 순정 Z 사본에서 한글패치 v5.1(182) → Z GUI(8, 겹침 3) → GUI 제거 →
KR 제거. 검증 실행은 modkit 소스를 C: 임시로 복사해 Windows 파이썬으로 —
WSL 경유 수 분짜리가 **총 4초**. 결과: GUI 설치 때 겹침 3장이 .pre-Z GUI로
보관, GUI 제거가 KR판을 그대로 복원(해시 동일), KR 제거 후 잔재 0. 이 과정에서
코어 재설치를 층으로 오인하는 버그(뜻-왕복 바이트 차이 → 제거가 순정 대신 자기
판 복원)를 발견·수리(same_core로 자기 판별). 층 아래로 밀린 모드는 부분이 아니라
설치됨으로 표시(보관 실물 대조).

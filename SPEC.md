...
코어(`Scripts.rxdata`)는 두 길 어느 쪽으로 되돌리든 **되돌릴 내용에 지금 살아
있는 주입 섹션을 도로 꽂는다**(`modassets._restored` → `merge_core`). 병합을
`.orig` 쪽에만 걸었던 판은, 층(`.pre-`)으로 간 제거가 층을 뜬 시점의 코어를
그대로 복원해 그 뒤에 얹힌 주입을 지웠다. 주입형이 남기는 백업 이름
(`Scripts.rxdata.orig`)이 에셋 백업과 같아, 주입이 하나라도 있는 게임에서는
코어 교체 설치가 늘 층을 뜨기 때문에 이 길이 기본값에 가깝다.

두 모드가 같은 자리에 **바이트까지 같은 파일**을 들고 오면 층이 안 뜬다 — 설치가
「내용이 같다」로 쓰기를 건너뛰기 때문이다. 그래서 자리마다 누가 넣었는지를 게임
폴더의 **소유 장부**(`modkit-owners.json`, 자리 → 모드 이름들)에 적는다. 설치가
자기 이름을 올리고 제거가 내린다. 규칙 둘:

- 아직 남이 쓰는 자리는 손대지 않는다. 내 `.pre-` 층이 있으면 그것(= 아래층의 판)을
  되살리고, 없으면 파일도 백업도 그대로 두고 안내한다(`warnings`).
- 마지막 소유자가 빠지면 순정으로 돌아간다. 이때 남아 있는 내 `.pre-` 층은 이미 빠진
  모드의 판이므로 되살리지 않고 버린다 — 되살리면 그 모드가 유령으로 남는다.

소유 판정에 파일 대조를 쓰면 안 된다. 보관소에 같은 그림을 든 모드가 넷 있으면
설치한 적 없는 셋까지 소유자로 읽혀 제거가 아무것도 못 되돌린다(2026-08-07 QA,
345자리). 장부가 없는 옛 설치본은 이 보호 없이 예전대로 돈다.

백업은 **자리마다 하나**이고 주인 이름이 없다(층은 `.pre-<모드명>`으로 주인이
붙는데 `.orig`는 안 붙는다). 그래서 한 자리를 함께 쓰는 두 모드 중 먼저 빠지는
쪽이 백업을 되돌리고 지우면, 나중 모드에게는 백업 없는 자리만 남는다. 이때
「백업이 없으니 내가 새로 놓은 파일」로 읽으면 순정이 지워진다. 그래서 제거는
카드에 `replaces_crc`가 있는 자리 — 원래 순정 파일이 있던 자리 — 는 백업이
없으면 **지우지 않고 그대로 두고 안내한다**(`warnings`). 지문이 없는 옛 카드는
이 보호를 못 받는다.
...
1. **차단** — 이미 설치된 다른 모드가 나를 `conflicts`로 지목하거나, 내가 설치된
   것을 `conflicts`로 지목하면 `declare.Blocked`를 던지고 멈춘다.
   `apply(..., force=True)`면 막는 대신 "강행: …" 문구로 경고에 담아 통과시킨다.

   `requires`는 **막지 않는다**(소프트 블락). 설치된 모드 이름에도 그들의
   `provides`에도 없으면 무엇이 없어 무슨 일이 날 수 있는지를 경고 한 줄로 알리고
   설치는 통과시킨다. 능력 이름의 표준 목록이 없어 철자 하나가 조용한 차단이
   되는 쪽이 더 나쁘다는 판단이다(2026-08-07 유지자 지시). 채워졌으면 아무 말도
   하지 않는다.
2. **자동 배치** — 두 단계다. 묶음형은 꽂을 자리를 먼저 고른다(`declare.place`):
   `order.after`/`order.before`가 가리키는 이름들의 현재 인덱스로 삽입 구간
   `[lo, hi]`를 구하고, `lo > hi`면 순환(모순)으로 보아 `Blocked`. 주입형은 늘
   `Main` 앞 일괄 삽입이라 자리를 고르지 않는다.

   자리를 고른 뒤(또는 꽂은 뒤) **설치본 전체를 선언대로 다시 늘어놓는다**
   (`declare.arrange`) — 묶음형은 항목 순서를, 주입형은 코어의 `MOD:` 섹션 순서를.
   그래서 상대 모드가 나중에 들어와도 그 자리에서 순서가 잡히고, 사람이 순서를
   외워 다시 설치할 일이 없다. 선언 없는 모드끼리는 지금 순서를 그대로 지킨다
   (안정 정렬). 재배치에서 순환을 만나면 **막지 않고** 지금 순서를 둔 채 경고만
   준다 — 이미 얹힌 뒤라 막아 봐야 설치는 끝나 있다. 제거 뒤에는 다시 늘어놓지
   않는다 — 상대가 빠지면 제약이 느슨해질 뿐이라 남은 순서는 여전히 선언을
   만족한다. 에셋 파일의 덮어쓰기 순서는 아직 `order`를 따르지 않는다
   (`docs/paths-in-pairs.md`의 남은 비대칭).
